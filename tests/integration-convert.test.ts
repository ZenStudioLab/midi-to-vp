...
describe("integration: conversion pipeline", () => {
  ...
  it("keeps slot parity across notation modes", () => {
    const extended = convertMidiToVp(createMidiFixture(), {
      notationMode: "extended",
    });
    const standard = convertMidiToVp(createMidiFixture(), {
  ...
  it("serializes standard mode without dash placeholders for empty slots", () => {
    const standardResult = convertMidiToVp(createMidiFixture(), {
      notationMode: "standard",
    });
  ...
  it("output in-range ratio reflects post-transform notes, not pre-transform source notes", () => {
    ...
    const sourceInRangeRatio =
      qualitySignals.inRangeNotes / qualitySignals.totalRawNotes;
    const outputInRangeRatio =
      qualitySignals.outputInRangeNotes! / qualitySignals.outputTotalNotes!;

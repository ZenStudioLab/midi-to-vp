...
export const ARTIFACT_DETECTION_CONFIG = {
  densitySpikeRatioThreshold: 3,
  densityCapMultiplier: 3,
  chordSpikeRatioThreshold: 3,
  chordCapMultiplier: 2,
} as const;
...
function resolveScoredPopulation(input: QualitySignals) {
  ...
  if (hasOutputTotalNotes !== hasOutputInRangeNotes) {
    throw new RangeError(
      "QualitySignals.outputTotalNotes and outputInRangeNotes must be provided together",
    );
  }
...
function validateSignals(input: QualitySignals): void {
  const entries = Object.entries(input) as Array<
    [keyof QualitySignals, number]
  >;

  for (const [field, value] of entries) {
    if (!Number.isFinite(value)) {
      throw new RangeError(
        `QualitySignals.${field} must be finite; received ${value}`,
      );
    }

    if (value < 0) {
      throw new RangeError(
        `QualitySignals.${field} must be >= 0; received ${value}`,
      );
    }
  }
  ...
  if (input.gridConfidence > 1) {
    throw new RangeError(
      `QualitySignals.gridConfidence must be <= 1; received ${input.gridConfidence}`,
    );
  }
}
...
function buildStats(
  ...
  const durationSeconds =
    input.avgNotesPerSecond > 0
      ? scoredPopulation.totalNotes / input.avgNotesPerSecond
      : 0;
...
export function normalizeSignals(input: QualitySignals): QualitySignalSet {
  ...
  const inRangeRatio = clamp01(
    scoredPopulation.inRangeNotes / scoredPopulation.totalNotes,
  );
  ...
  const p95DensityPenalty = clamp01(
    Math.max(0, input.p95NotesPerSecond - 8) / 8,
  );
  const maxDensityPenalty = clamp01(
    Math.max(0, input.maxNotesPerSecond - 12) / 12,
  );
  const noteDensity = clamp01(
    1 -
      (avgDensityPenalty * 0.35 +
        p95DensityPenalty * 0.4 +
        maxDensityPenalty * 0.25),
  );
...
export function getEffectiveQualityWeights(gridConfidence: number) {
  ...
  const effectiveTimingWeight =
    SCORING_WEIGHTS.timingConsistency * normalizedConfidence;
  const redistributed =
    SCORING_WEIGHTS.timingConsistency - effectiveTimingWeight;
  ...
  return {
    ...
    chordComplexity: Number(
      (SCORING_WEIGHTS.chordComplexity * scale).toFixed(12),
    ),
...
export function hasFatalReason(reasons: ReasonCode[]): boolean {
  return reasons.some(
    (reason) =>
      reason.startsWith("FATAL_") || reason.startsWith("INPUT_LIMIT_EXCEEDED_"),
  );
}

export function createValidationFailureAssessment(
  reason: ValidationFailureReasonCode,
): ScoringAssessment {
  return {
    score: 0,
...
export function applyArtifactNormalization(signals: QualitySignals): {
  maxNotesPerSecond: number;
  peakChordSize: number;
  artifactCapped: ArtifactCapped;
} {
  const artifactCapped: ArtifactCapped = {
    maxNotesPerSecond: false,
    peakChordSize: false,
  };

  let normalizedMaxNotesPerSecond = signals.maxNotesPerSecond;
  let normalizedPeakChordSize = signals.peakChordSize;

  const {
    densitySpikeRatioThreshold: densityThreshold,
    densityCapMultiplier: densityMultiplier,
    chordSpikeRatioThreshold: chordThreshold,
    chordCapMultiplier: chordMultiplier,
  } = ARTIFACT_DETECTION_CONFIG;

  if (signals.p95NotesPerSecond > 0) {
    const densityRatio = signals.maxNotesPerSecond / signals.p95NotesPerSecond;
    if (densityRatio > densityThreshold) {
      normalizedMaxNotesPerSecond = Math.min(
        signals.maxNotesPerSecond,
        signals.p95NotesPerSecond * densityMultiplier,
      );
      artifactCapped.maxNotesPerSecond = true;
    }
  }

  if (signals.p95ChordSize > 0) {
    const chordRatio = signals.peakChordSize / signals.p95ChordSize;
    if (chordRatio > chordThreshold) {
      normalizedPeakChordSize = Math.min(
        signals.peakChordSize,
        signals.p95ChordSize * chordMultiplier,
      );
      artifactCapped.peakChordSize = true;
    }
  }

  return {
    maxNotesPerSecond: normalizedMaxNotesPerSecond,
    peakChordSize: normalizedPeakChordSize,
    artifactCapped,
  };
}

export function scoreConversionQuality(
  input: QualitySignals,
): ScoringAssessment {
  const { maxNotesPerSecond, peakChordSize, artifactCapped } =
    applyArtifactNormalization(input);
  const reasonSignals: QualitySignals = {
    ...input,
    maxNotesPerSecond,
    peakChordSize,
  };

  // Plan constraint: artifact normalization affects reasons only.
  const signals = normalizeSignals(input);
  const stats = buildStats(input, artifactCapped);
  const reasons = collectReasons(reasonSignals, signals);
